...
object UnearnedIncomeSourceHandler
  ...
  override def summaryHandler(
    ...
    summaryType match {
      case SavingsIncomes =>
        Some(SummaryHandler(new SandboxSummaryRepository[SavingsIncome] {
          override def example(id: Option[SummaryId]) =
            SavingsIncome.example(id)
          override implicit val writes = SavingsIncome.writes
        }, SavingsIncome, SavingsIncomes.name))
      case Dividends =>
        Some(SummaryHandler(new SandboxSummaryRepository[Dividend] {
  ...
  override val repository = new SandboxSourceRepository[UnearnedIncome] {
    ...
    override def example(id: SourceId) =
      UnearnedIncome.example().copy(id = Some(id))
  }
}
