...
import uk.gov.hmrc.selfassessmentapi.controllers.api.unearnedincome.SummaryTypes.{
  Benefits,
  Dividends,
  SavingsIncomes
}
...
object UnearnedIncomeSourceHandler
    extends SourceHandler(UnearnedIncome, UnearnedIncomes.name) {
      ...
      case SavingsIncomes =>
        Some(
          SummaryHandler(SummaryRepositoryWrapper(
                           UnearnedIncomeRepository().SavingsIncomeRepository),
                         SavingsIncome,
                         SavingsIncomes.name))
      case Dividends =>
        Some(
          SummaryHandler(SummaryRepositoryWrapper(
                           UnearnedIncomeRepository().DividendRepository),
                         Dividend,
                         Dividends.name))
      case Benefits =>
        Some(
          SummaryHandler(SummaryRepositoryWrapper(
                           UnearnedIncomeRepository().BenefitRepository),
                         Benefit,
                         Benefits.name))
      case _ =>
        throw new NotImplementedException(
          s"${UnearnedIncomes.name} ${summaryType.name} is not implemented")
    }
  }
