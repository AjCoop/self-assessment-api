...
object FurnishedHolidayLettingsSourceHandler
    extends SourceHandler(FurnishedHolidayLetting,
                          FurnishedHolidayLettings.name) {
    ...
    summaryType match {
      case Incomes =>
        Some(
          SummaryHandler(
            SummaryRepositoryWrapper(
              FurnishedHolidayLettingsRepository().IncomeRepository),
            Income,
            Incomes.name))
      case Expenses =>
        Some(
          SummaryHandler(
            SummaryRepositoryWrapper(
              FurnishedHolidayLettingsRepository().ExpenseRepository),
            Expense,
            Expenses.name))
      case BalancingCharges =>
        Some(
          SummaryHandler(
            SummaryRepositoryWrapper(
              FurnishedHolidayLettingsRepository().BalancingChargeRepository),
            BalancingCharge,
            BalancingCharges.name))
      case PrivateUseAdjustments =>
        Some(SummaryHandler(
          SummaryRepositoryWrapper(
            FurnishedHolidayLettingsRepository().PrivateUseAdjustmentRepository),
          PrivateUseAdjustment,
          PrivateUseAdjustments.name))
      case _ =>
        throw new NotImplementedException(
          s"${FurnishedHolidayLettings.name} ${summaryType.name} is not implemented")
    }
  }

  override val repository = SourceRepositoryWrapper(
