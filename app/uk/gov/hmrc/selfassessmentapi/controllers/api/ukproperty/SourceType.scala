...
object SourceType {
  ...
  case object UKProperties extends SourceType {
    ...
    override def example(sourceId: Option[SourceId] = None): JsValue =
      toJson(UKProperty.example(sourceId))
    ...
    override val fieldDescriptions = Seq(
      ...
      PositiveMonetaryFieldDescription(
        ...
        """Annual Investment Allowance can be claimed if equipment was purchased (but not cars) during the year.
          ...
          |AIA can’t be claimed for expenditure on equipment and other items for use in a dwelling house""".stripMargin,
        optional = true),
      PositiveMonetaryFieldDescription(
        name,
        "businessPremisesRenovationAllowance",
        "When eligible, BPRA can be claimed for the cost of renovating or repairing business premises",
        optional = true),
      PositiveMonetaryFieldDescription(
        name,
        "otherCapitalAllowance",
        "The type of capital allowance and amount that can be claimed will depend on the cost, type of asset and other circumstances",
        optional = true),
      PositiveMonetaryFieldDescription(
        name,
        ...
        """For fully furnished accommodation, wear & tear allowance can be claimed up to 10%
          ...
          | tenant would usually pay for but which are paid by you (such as Council Tax)""".stripMargin,
        optional = true),
      FullFieldDescription(name,
                           "adjustments",
                           "Object",
      ...
      PositiveMonetaryFieldDescription(
        ...
        """For claiming the exempt amount (either £4,250 or £2,125, if let jointly)
          | if any Rent a Room income has been included within Rental Income""".stripMargin,
        optional = true)
    )
  }
