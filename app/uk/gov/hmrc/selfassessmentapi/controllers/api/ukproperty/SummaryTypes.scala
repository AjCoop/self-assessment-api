...
object SummaryTypes {
  ...
  case object Incomes extends SummaryType {
    ...
    override def description(action: String) =
      s"$action an income for the specified UK Property"
    override val fieldDescriptions = Seq(
      ...
      PositiveMonetaryFieldDescription(
        "uk-properties",
        "amount",
        "Income from all UK property & land (does not include any income/profits from FHL). Split by income type")
    )
  }
  ...
  case object Expenses extends SummaryType {
    ...
    override def description(action: String) =
      s"$action an expense for the specified UK Property"
    override val fieldDescriptions = Seq(
      FullFieldDescription(
        "uk-property",
        "type",
      ...
      PositiveMonetaryFieldDescription(
        "uk-properties",
        "amount",
        "Running costs associated with the rental business, split by expense type")
    )
  }
  ...
  case object TaxesPaid extends SummaryType {
    ...
    override def example(id: Option[SummaryId] = None): JsValue =
      toJson(TaxPaid.example(id))
    override val title = "Sample UK property taxes paid"
    override def description(action: String) =
      s"$action a tax paid for the specified UK Property"
    override val fieldDescriptions = Seq(
      PositiveMonetaryFieldDescription(
        "uk-properties",
        "amount",
        "For non-resident landlords - amount of tax taken off any property rental income.")
    )
  }
  ...
  case object BalancingCharges extends SummaryType {
    ...
    override def description(action: String) =
      s"$action a balancing charge for the specified UK Property"
    override val fieldDescriptions = Seq(
      PositiveMonetaryFieldDescription(
        "uk-properties",
        "amount",
        "For items where capital allowances were claimed that have been sold," +
  ...
  case object PrivateUseAdjustments extends SummaryType {
    ...
    override def description(action: String) =
      s"$action a private use adjustment for the specified UK Property"
    override val fieldDescriptions = Seq(
      PositiveMonetaryFieldDescription(
        "uk-properties",
        "amount",
        "The private use (non-business) amount of any expenses " +
