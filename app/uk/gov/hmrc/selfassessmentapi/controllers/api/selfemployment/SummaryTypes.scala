...
object SummaryTypes {
  ...
  case object Incomes extends SummaryType {
    ...
    override val fieldDescriptions = Seq(
      FullFieldDescription(
        ...
        "type",
        "Enum",
        s"Type of income (one of the following: ${IncomeType.values.mkString(", ")})"),
      PositiveMonetaryFieldDescription(
        "self-employments",
        "amount",
        "Income from the business including turnover (from takings, fees & sales) earned or received by the business before expenses, and other business income not included within turnover.")
    )
  }
  ...
  case object Expenses extends SummaryType {
    ...
    override val fieldDescriptions = Seq(
      ...
      PositiveMonetaryFieldDescription(
        "self-employments",
        "amount",
        "Allowable expenses associated with the running of the business, split by expense type")
    )
  }
  ...
  case object BalancingCharges extends SummaryType {
    ...
    override def description(action: String) =
      s"$action a balancing charge for the specified source"
    override val fieldDescriptions = Seq(
      FullFieldDescription(
        "self-employments",
        "type",
        "Enum",
        s"Type of balancing charge (one of the following: ${BalancingChargeType.values
          .mkString(", ")})"),
      PositiveMonetaryFieldDescription(
        "self-employments",
        "amount",
        "If an item has been sold where capital allowance was claimed, and the sale or value of the item is more than the pool value or cost, " +
  ...
  case object GoodsAndServicesOwnUses extends SummaryType {
    ...
    override def example(id: Option[SummaryId] = None): JsValue =
      toJson(GoodsAndServicesOwnUse.example(id))
    override val title = "Sample self-employment goods and service for own use"
    override def description(action: String) =
      s"$action a goods and service for own use summary for the specified source"
    override val fieldDescriptions = Seq(
      PositiveMonetaryFieldDescription(
        "self-employments",
        "amount",
        "To account for the normal sale price of goods or stock have been taken out of the business.")
    )
  }
