...
object SummaryTypes {
  ...
  case object SavingsIncomes extends SummaryType {
    ...
    override def example(id: Option[SummaryId] = None): JsValue =
      toJson(SavingsIncome.example(id))
    override val title = "Sample unearned income savings incomes"
    override def description(action: String) =
      s"$action a savings income for the specified source"
    override val fieldDescriptions = Seq(
      FullFieldDescription(
        "unearned income",
        "type",
        "Enum",
        s"Type of savings income (one of the following: ${SavingsIncomeType.values
          .mkString(", ")})"),
      PositiveMonetaryFieldDescription(
        "unearned income",
        "amount",
        "Interest income from UK banks and building societies, split by interest types - Taxed interest and Untaxed interest.")
    )
  }
  ...
  case object Dividends extends SummaryType {
    ...
    override def example(id: Option[SummaryId] = None): JsValue =
      toJson(Dividend.example(id))
    override val title = "Sample unearned income dividends"
    override def description(action: String) =
      s"$action a dividend for the specified source"
    override val fieldDescriptions = Seq(
      FullFieldDescription(
        "unearned income",
        "type",
        "Enum",
        s"Type of dividends income (one of the following: ${DividendType.values
          .mkString(", ")})"),
      PositiveMonetaryFieldDescription(
        "unearned income",
        "amount",
        "Dividend income from the UK sources, split by dividend types - Dividend from UK companies and Other Dividends.")
    )
  }
  ...
  case object Benefits extends SummaryType {
    ...
    override val fieldDescriptions = Seq(
      ...
      PositiveMonetaryFieldDescription(
        "unearned income",
        "amount",
        "Pension, annuities and state benefits from UK, split by type - state pension, state pension lump sum, Other pensions, " +
