...
import uk.gov.hmrc.selfassessmentapi.controllers.live.LiabilityController.{
  NotFound => _,
  NotImplemented => _
}
...
object ControllerConfiguration {
  ...
  implicit val regexValueReader: ValueReader[Regex] =
    StringReader.stringValueReader.map(_.r)

  implicit val controllerParamsReader =
...
object MicroserviceAuthFilter extends AuthorisationFilter {
  ...
  private def extractIdentityResource(
      pathString: String,
      verb: HttpVerb,
      authConfig: AuthConfig): Option[ResourceToAuthorise] = {
    pathString match {
  ...
  override def apply(next: (RequestHeader) => Future[Result])(
    ...
    super.apply(next)(rh) map { res =>
      res.header.status match {
        case 401 =>
          Status(ErrorUnauthorized.httpStatusCode)(
  ...
  override def controllerNeedsAuth(controllerName: String): Boolean =
    AppContext.authEnabled && ControllerConfiguration
      .controllerParamsConfig(controllerName)
      .needsAuth
}
...
object HeaderValidatorFilter extends Filter with HeaderValidator {
  def apply(next: (RequestHeader) => Future[Result])(
    ...
    val needsHeaderValidation = controller
      .map(
        name =>
...
object MicroserviceGlobal
  ...
  override def microserviceFilters: Seq[EssentialFilter] =
    Seq(HeaderValidatorFilter) ++ defaultMicroserviceFilters
  ...
  override def onError(request: RequestHeader, ex: Throwable) = {
    super.onError(request, ex).map { result =>
      ex.getCause match {
        case ex: UnknownSummaryException =>
  ...
  override def onBadRequest(request: RequestHeader, error: String) = {
    super.onBadRequest(request, error).map { result =>
      error match {
        case "ERROR_INVALID_SOURCE_TYPE" =>
          NotFound(Json.toJson(ErrorNotFound))
        case "ERROR_TAX_YEAR_INVALID" =>
          BadRequest(
            Json.toJson(
              ErrorBadRequest(ErrorCode.TAX_YEAR_INVALID, "Tax year invalid")))
        case _ => result
      }
    }
  }
