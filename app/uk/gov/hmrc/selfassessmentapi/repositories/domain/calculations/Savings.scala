...
import uk.gov.hmrc.selfassessmentapi.controllers.api.savings.IncomeType
...
object Savings {
  ...
  object TotalTaxPaid {
    def apply(selfAssessment: SelfAssessment): BigDecimal = {
      selfAssessment.savings.map { saving =>
        RoundUpToPennies(TaxedInterest(saving) * 0.25)
      }.sum
    }
  }
  ...
  private object Interest {
    def apply(saving: Saving, `type`: IncomeType.Value) = saving.incomes.filter(_.`type` == `type`).map(_.amount).sum
  }

  private object TaxedInterest {
    def apply(mongoSavings: Saving) = Interest(mongoSavings, IncomeType.InterestFromBanksTaxed)
  }

  private object UntaxedInterest {
    def apply(mongoSavings: Saving) = Interest(mongoSavings, IncomeType.InterestFromBanksUntaxed)
  }

  object Incomes {
    def apply(selfAssessment: SelfAssessment): Seq[InterestFromUKBanksAndBuildingSocieties] =
      selfAssessment.savings.map { saving =>
        new InterestFromUKBanksAndBuildingSocieties(saving.sourceId, RoundDown(TaxedInterest(saving) * 100/80 + UntaxedInterest(saving)))
      }
  }
