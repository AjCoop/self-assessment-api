...
case class UKPropertyBuilder(rentARoomRelief: BigDecimal = 0,
  ...
  private var ukProperties: UKProperties = UKProperties(objectID,
                                                        objectID.stringify,
                                                        generateSaUtr(),
                                                        taxYear,
                                                        now,
                                                        now,
                                                        Some(rentARoomRelief),
                                                        Some(Allowances()),
                                                        Some(Adjustments()))

  def withAllowances(annualInvestmentAllowance: BigDecimal,
                     otherCapitalAllowance: BigDecimal,
                     wearAndTearAllowance: BigDecimal,
                     businessPremisesRenovationAllowance: BigDecimal) = {
    ukProperties = ukProperties.copy(
      allowances = ukProperties.allowances.map(
        _.copy(otherCapitalAllowance = Some(otherCapitalAllowance),
               annualInvestmentAllowance = Some(annualInvestmentAllowance),
               wearAndTearAllowance = Some(wearAndTearAllowance),
               businessPremisesRenovationAllowance =
                 Some(businessPremisesRenovationAllowance))))
    this
  }
  ...
  def incomes(incomes: (IncomeType.IncomeType, BigDecimal)*) = {
    ukProperties = ukProperties.copy(incomes = incomes.map(income =>
      UKPropertiesIncomeSummary("", income._1, income._2)))
    this
  }

  def expenses(expenses: (ExpenseType.ExpenseType, BigDecimal)*) = {
    ukProperties = ukProperties.copy(expenses = expenses.map(expense =>
      UKPropertiesExpenseSummary("", expense._1, expense._2)))
    this
  }

  def balancingCharges(amounts: BigDecimal*) = {
    ukProperties = ukProperties.copy(balancingCharges = amounts.map(amount =>
  ...
  def privateUseAdjustment(amounts: BigDecimal*) = {
    ukProperties =
      ukProperties.copy(privateUseAdjustment = amounts.map(amount =>
        UKPropertiesPrivateUseAdjustmentSummary("", amount)))
    this
  }

  def taxesPaid(amounts: BigDecimal*) = {
    ukProperties = ukProperties.copy(taxesPaid = amounts.map(amount =>
      UKPropertiesTaxPaidSummary("", amount)))
    this
  }
