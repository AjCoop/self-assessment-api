...
case class SelfEmploymentBuilder(
      ...
      allowances = selfEmployment.allowances.map(
        _.copy(allowancesOnSales = Some(allowancesOnSales),
               ...
               capitalAllowanceSpecialRatePool =
                 Some(capitalAllowanceSpecialRatePool),
  ...
  def incomes(incomes: (IncomeType.IncomeType, BigDecimal)*) = {
    selfEmployment = selfEmployment.copy(incomes = incomes.map(income =>
  ...
  def expenses(expenses: (ExpenseType.ExpenseType, BigDecimal)*) = {
    selfEmployment = selfEmployment.copy(
      expenses = expenses.map(
        expense =>
          SelfEmploymentExpenseSummary(objectID.stringify,
                                       expense._1,
                                       expense._2)))
    this
  }

  def balancingCharges(
      balancingCharges: (BalancingChargeType.BalancingChargeType, BigDecimal)*) = {
    selfEmployment = selfEmployment.copy(
      balancingCharges = balancingCharges.map(
        balancingCharge =>
          SelfEmploymentBalancingChargeSummary(objectID.stringify,
                                               balancingCharge._1,
                                               balancingCharge._2)))
    this
  }
  ...
  def goodsAndServicesOwnUse(amounts: BigDecimal*) = {
    selfEmployment = selfEmployment.copy(
      goodsAndServicesOwnUse = amounts.map(
