...
class SavingsSpec extends UnitSpec {
  ...
  "Interest from UK banks and building societies" should {
    ...
    "calculate rounded down interest when there are multiple interest of both taxed and unTaxed from uk banks and building societies from" +
      ...
      val savingsOne = SavingBuilder()
        .withTaxedIncome(100.50)
        .withUntaxedIncome(200.50)
        .create()

      val savingsTwo = SavingBuilder()
        .withTaxedIncome(300.99)
        .withUntaxedIncome(400.99)
        .create()

      Savings.Incomes(SelfAssessment(savings = Seq(savingsOne, savingsTwo))) should contain theSameElementsAs
        Seq(api.InterestFromUKBanksAndBuildingSocieties(sourceId = savingsOne.sourceId, BigDecimal(326)),
          api.InterestFromUKBanksAndBuildingSocieties(sourceId = savingsTwo.sourceId, BigDecimal(777)))
    }
    ...
    "calculate interest when there is one taxed interest from uk banks and building societies from a single unearned income source" in {
      val saving = SavingBuilder()
        .withTaxedIncome(100)
        .create()

      Savings.Incomes(SelfAssessment(savings = Seq(saving))) should contain theSameElementsAs
        Seq(api.InterestFromUKBanksAndBuildingSocieties(saving.sourceId, BigDecimal(125)))
    ...
    "calculate interest when there are multiple taxed interest from uk banks and building societies from a single unearned income source" in {
      val saving = SavingBuilder()
        .withTaxedIncome(100, 200)
        .create()

      Savings.Incomes(SelfAssessment(savings = Seq(saving))) should contain theSameElementsAs
        Seq(api.InterestFromUKBanksAndBuildingSocieties(saving.sourceId, BigDecimal(375)))
    }
    ...
    "calculate round down interest when there are multiple taxed interest from uk banks and building societies from a single unearned " +
      "income source" in {
      val saving = SavingBuilder()
        .withTaxedIncome(100.90, 200.99)
        .create()

      Savings.Incomes(SelfAssessment(savings = Seq(saving))) should contain theSameElementsAs
        Seq(api.InterestFromUKBanksAndBuildingSocieties(saving.sourceId, BigDecimal(377)))
    ...
    "calculate interest when there is one unTaxed interest from uk banks and building societies from a single unearned income source" in {
      val saving = SavingBuilder()
        .withUntaxedIncome(100)
        .create()

      Savings.Incomes(SelfAssessment(savings = Seq(saving))) should contain theSameElementsAs
        Seq(InterestFromUKBanksAndBuildingSocieties(saving.sourceId, BigDecimal(100)))
    ...
    "calculate interest when there are multiple unTaxed interest from uk banks and building societies from a single unearned income " +
      "source" in {
      val saving = SavingBuilder()
        .withUntaxedIncome(100, 200)
        .create()

      Savings.Incomes(SelfAssessment(savings = Seq(saving))) should contain theSameElementsAs
        Seq(api.InterestFromUKBanksAndBuildingSocieties(saving.sourceId, BigDecimal(300)))
    }
    ...
    "calculate rounded down interest when there is one unTaxed interest from uk banks and building societies from a single unearned " +
      "income source" in {
      val saving = SavingBuilder()
        .withUntaxedIncome(100.50)
        .create()

      Savings.Incomes(SelfAssessment(savings = Seq(saving))) should contain theSameElementsAs
        Seq(api.InterestFromUKBanksAndBuildingSocieties(saving.sourceId, BigDecimal(100)))
    }
    ...
    "calculate rounded down interest when there are multiple unTaxed interest from uk banks and building societies from a single unearned" +
      " income source" in {
      val saving = SavingBuilder()
        .withUntaxedIncome(100.50, 200.99)
        .create()

      Savings.Incomes(SelfAssessment(savings = Seq(saving))) should contain theSameElementsAs
        Seq(api.InterestFromUKBanksAndBuildingSocieties(saving.sourceId, BigDecimal(301)))
    }
  }
  ...
  "Savings.TotalTaxPaid" should {
    ...
    "be 0 if sum of all taxed interests is 0" in {
      ...
      Savings.TotalTaxPaid(SelfAssessment(savings = Seq(saving))) shouldBe 0
    }

    "be equal to Sum(Taxed Interest) * 100/80 - Sum(Taxed Interest)" in {

      val savingOne = SavingBuilder()
        .withTaxedIncome(100, 200, 2000)
        .withUntaxedIncome(500)
        .create()

      Savings.TotalTaxPaid(SelfAssessment(savings = Seq(savingOne))) shouldBe 575

      val savingTwo = SavingBuilder()
        .withTaxedIncome(400, 700, 5800)
        .withUntaxedIncome(500)
        .create()

      Savings.TotalTaxPaid(SelfAssessment(savings = Seq(savingTwo))) shouldBe 1725
    ...
    "be equal to RoundUpToPennies(RoundUp(Sum(Taxed Interest)) * 100/80 - Sum(Taxed Interest))" in {
      ...
      val savingOne = SavingBuilder()
        .withTaxedIncome(786.78, 456.76, 2000.56)
        .withUntaxedIncome(1000.56)
        .create()

      Savings.TotalTaxPaid(SelfAssessment(savings = Seq(savingOne))) shouldBe 811.03

      val savingTwo = SavingBuilder()
        .withTaxedIncome(1000.78, 999.22, 3623.67)
        .withUntaxedIncome(2000.56)
        .create()

      Savings.TotalTaxPaid(SelfAssessment(savings = Seq(savingTwo))) shouldBe 1405.92
    }
    ...
    "be equal to RoundUpToPennies(RoundUp(Sum(Taxed Interest)) * 100/80 - Sum(Taxed Interest)) for multiple unearned income sources" in {
      val savingOne = SavingBuilder()
        .withTaxedIncome(786.78)
        .withUntaxedIncome(2500.00)
        .create()

      val savingTwo = SavingBuilder()
        .withTaxedIncome(456.76, 2000.56)
        .withUntaxedIncome(2500.00)
        .create()

      Savings.TotalTaxPaid(SelfAssessment(savings = Seq(savingOne, savingTwo))) shouldBe 811.03
    }
  }
  ...
  "Savings.IncomeTaxBandSummary" should {
    "be calculated when NonSavingsIncome = 0 and TaxableSavingIncome falls within BasicRate band" in {
      Savings.IncomeTaxBandSummary(SelfAssessmentBuilder()
        .withSavings(SavingBuilder().withUntaxedIncome(42999))
        .create()
    ...
    "be calculated when NonSavingsIncome = 0 and TaxableSavingIncome is spread over Basic and Higher Rate band" in {
      Savings.IncomeTaxBandSummary(SelfAssessmentBuilder()
        .withSavings(SavingBuilder().withUntaxedIncome(43001))
        .create()
      ) should contain theSameElementsInOrderAs
        Seq(
          TaxBandSummary("startingRate", 5000.0, "0%", 0.0),
    ...
    "be calculated when NonSavingsIncome = 0 and TaxableSavingIncome is spread over Basic, Higher and Additional Higher Rate band" in {
      Savings.IncomeTaxBandSummary(SelfAssessmentBuilder()
        .withSavings(SavingBuilder().withUntaxedIncome(150001))
        .create()
      ) should contain theSameElementsInOrderAs
        Seq(
          TaxBandSummary("startingRate", 5000.0, "0%", 0.0),
    ...
    "be calculated when NonSavingsIncome > 0 and TaxableSavingIncome falls within BasicRate band" in {
      ...
      Savings.IncomeTaxBandSummary(SelfAssessmentBuilder()
        .withSavings(SavingBuilder().withUntaxedIncome(30999))
    ...
    "be calculated when NonSavingsIncome > 0 and TaxableSavingIncome is spread over Basic and Higher Rate band" in {
      Savings.IncomeTaxBandSummary(SelfAssessmentBuilder()
        .withSavings(SavingBuilder().withUntaxedIncome(31001))
    ...
    "be calculated when NonSavingsIncome > 0 and TaxableSavingIncome is spread over Basic, Higher and Additional Higher Rate band" in {
      ...
      Savings.IncomeTaxBandSummary(SelfAssessmentBuilder()
        .withSavings(SavingBuilder().withUntaxedIncome(149001))
    ...
    "be allocated to correct tax bands" in {
      ...
      TableDrivenPropertyChecks.forAll(inputs) { (totalProfitFromSelfEmployments: String, totalSavingsIncome: String, ukPensionContributions: String,
        ...
        val bandAllocations = Savings.IncomeTaxBandSummary(SelfAssessmentBuilder()
          .withSavings(SavingBuilder().withUntaxedIncome(totalSavingsIncome.toInt))
    ...
    "be equal to" in {
      ...
      TableDrivenPropertyChecks.forAll(inputs) { (nonSavingsIncome: String, savingsIncome: String, ukPensionContributions: String, savingsIncomeTax: String) =>
        ...
        val savingsIncomeBandAllocation = Savings.IncomeTaxBandSummary(SelfAssessmentBuilder()
          .withSavings(SavingBuilder().withUntaxedIncome(savings))
