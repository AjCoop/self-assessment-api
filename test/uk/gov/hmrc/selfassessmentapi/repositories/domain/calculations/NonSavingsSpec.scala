...
class NonSavingsSpec extends UnitSpec {
  ...
  "NonSavings.TotalIncome" should {
    "be selfEmploymentProfits + employmentProfits + ukPropertyProfits + furnishedHolidayLettingProfits" in {
      NonSavings.TotalIncome(
        selfEmploymentProfits = 1000,
        employmentProfits = 2000,
  ...
  "NonSavings.TaxBandSummaries" should {
    ...
    "be calculated for total NonSavingsIncome < 32000" in {
      NonSavings.IncomeTaxBandSummary(totalNonSavingsTaxableIncome = 20000) should contain theSameElementsAs
        Seq(TaxBandSummary("basicRate", 20000.00, "20%", 4000.00),
      ...
      NonSavings.IncomeTaxBandSummary(totalNonSavingsTaxableIncome = 31999.00) should contain theSameElementsAs
        Seq(TaxBandSummary("basicRate", 31999.00, "20%", 6399.80),
    ...
    "be calculated for total NonSavingsIncome = 32000" in {
      NonSavings.IncomeTaxBandSummary(totalNonSavingsTaxableIncome = 32000) should contain theSameElementsAs
        Seq(TaxBandSummary("basicRate", 32000.00, "20%", 6400.00),
    ...
    "be calculated tax for 32000 < NonSavingsIncome < 150000 and pension contributions" in {
      NonSavings.IncomeTaxBandSummary(
        totalNonSavingsTaxableIncome = 32001,
        ukPensionContributions = 100) should contain theSameElementsAs
        Seq(TaxBandSummary("basicRate", 32001.00, "20%", 6400.20),
            TaxBandSummary("higherRate", 0, "40%", 0),
            TaxBandSummary("additionalHigherRate", 0, "45%", 0))
    }
    ...
    "be calculated tax for NonSavingsIncome > 150000 and pension contributions" in {
      NonSavings.IncomeTaxBandSummary(
        totalNonSavingsTaxableIncome = 150003,
        ukPensionContributions = 100) should contain theSameElementsAs
        Seq(TaxBandSummary("basicRate", 32100.00, "20%", 6420.00),
            TaxBandSummary("higherRate", 117903.00, "40%", 47161.20),
            TaxBandSummary("additionalHigherRate", 0, "45%", 0.00))

      NonSavings.IncomeTaxBandSummary(
        totalNonSavingsTaxableIncome = 160000,
        ukPensionContributions = 1000) should contain theSameElementsAs
        Seq(TaxBandSummary("basicRate", 33000.00, "20%", 6600.00),
            TaxBandSummary("higherRate", 118000.00, "40%", 47200.00),
  ...
  "NonSavingsIncomeTax" should {
    "be equal to" in {
      ...
      TableDrivenPropertyChecks.forAll(inputs) {
        (totalTaxableProfits: String, ukPensionContributions: String,
         nonSavingsIncomeTax: String) =>
          NonSavings.IncomeTax(
            NonSavings
              .IncomeTaxBandSummary(
                totalNonSavingsTaxableIncome =
                  BigDecimal(totalTaxableProfits.toInt),
                ukPensionContributions = BigDecimal(
                  ukPensionContributions.toInt))) shouldBe BigDecimal(
            nonSavingsIncomeTax.toDouble)
      }
    }
  }
}
