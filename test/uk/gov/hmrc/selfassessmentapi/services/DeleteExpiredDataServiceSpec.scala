...
class DeleteExpiredDataServiceSpec
  ...
  implicit override val patienceConfig =
    PatienceConfig(timeout = Span(5, Seconds), interval = Span(300, Millis))
  ...
  "deleteExpiredData" should {
    ...
    "delete only the expired records (older than the lastModifiedDate) and not the latest records which have not expired" in {
      ...
      val sa2 = SelfAssessment(BSONObjectID.generate,
                               ...
                               DateTime.now().minusMonths(2))
      val latestSa3 = SelfAssessment(BSONObjectID.generate,
                                     saUtr3,
                                     taxYear,
                                     DateTime.now().minusDays(1),
                                     DateTime.now().minusDays(1))

      val se1 = SelfEmployment.create(saUtr,
                                      taxYear,
                                      selfemployment.SelfEmployment.example())
      val se2 = SelfEmployment.create(saUtr2,
                                      taxYear,
                                      selfemployment.SelfEmployment.example())
      val latestSe3 = SelfEmployment
        .create(saUtr3, taxYear, selfemployment.SelfEmployment.example())

      insertSelfAssessmentRecords(sa1, sa2, latestSa3)
      insertSelfEmploymentRecords(se1, se2, latestSe3)

      val service =
        new DeleteExpiredDataService(saRepo, seRepo, uiRepo, jobRepo)
    ...
    "mark job as failed if there is an exception when trying to delete records from self assessment" in {
      ...
      an[RuntimeException] should be thrownBy await(
        service.deleteExpiredData(DateTime.now))
