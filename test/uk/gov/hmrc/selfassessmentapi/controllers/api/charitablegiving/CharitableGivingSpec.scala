...
package uk.gov.hmrc.selfassessmentapi.controllers.api.charitablegiving
  ...
  "validate" should {
    ...
    "reject amounts with more than 2 decimal values" in {
      Seq(BigDecimal(1000.123),
        ...
        assertValidationError[CharitableGiving](
          CharitableGiving(
            giftAidPayments =
              Some(GiftAidPayments(totalInTaxYear = Some(testAmount)))),
          Map("/giftAidPayments/totalInTaxYear" -> INVALID_MONETARY_AMOUNT)
        )
        assertValidationError[CharitableGiving](
          CharitableGiving(giftAidPayments =
            Some(GiftAidPayments(oneOff = Some(testAmount)))),
          Map("/giftAidPayments/oneOff" -> INVALID_MONETARY_AMOUNT)
        )
        assertValidationError[CharitableGiving](
          CharitableGiving(
            giftAidPayments =
              Some(GiftAidPayments(toNonUkCharities = Some(testAmount)))),
          Map("/giftAidPayments/toNonUkCharities" -> INVALID_MONETARY_AMOUNT)
        )
        assertValidationError[CharitableGiving](
          CharitableGiving(giftAidPayments = Some(
            GiftAidPayments(carriedBackToPreviousTaxYear = Some(testAmount)))),
          Map("/giftAidPayments/carriedBackToPreviousTaxYear" -> INVALID_MONETARY_AMOUNT)
        )
        assertValidationError[CharitableGiving](
          CharitableGiving(giftAidPayments =
            Some(GiftAidPayments(carriedFromNextTaxYear = Some(testAmount)))),
          Map("/giftAidPayments/carriedFromNextTaxYear" -> INVALID_MONETARY_AMOUNT)
        )
        assertValidationError[CharitableGiving](
          CharitableGiving(
            sharesSecurities =
              Some(SharesAndSecurities(totalInTaxYear = testAmount))),
          Map("/sharesSecurities/totalInTaxYear" -> INVALID_MONETARY_AMOUNT)
        )
        assertValidationError[CharitableGiving](
          CharitableGiving(
            sharesSecurities = Some(
              SharesAndSecurities(totalInTaxYear = 100,
                                  toNonUkCharities = Some(testAmount)))),
          Map("/sharesSecurities/toNonUkCharities" -> INVALID_MONETARY_AMOUNT)
        )
        assertValidationError[CharitableGiving](
          CharitableGiving(
            landProperties =
              Some(LandAndProperties(totalInTaxYear = testAmount))),
          Map("/landProperties/totalInTaxYear" -> INVALID_MONETARY_AMOUNT)
        )
        assertValidationError[CharitableGiving](
          CharitableGiving(
            landProperties = Some(
              LandAndProperties(totalInTaxYear = 100,
                                toNonUkCharities = Some(testAmount)))),
    ...
    "reject if oneOff is provided, but totalInTaxYear is not present" in {
      assertValidationError[CharitableGiving](
        CharitableGiving(
          giftAidPayments =
            Some(GiftAidPayments(totalInTaxYear = None, oneOff = Some(100)))),
      ...
      assertValidationError[CharitableGiving](
        CharitableGiving(
          giftAidPayments = Some(
            GiftAidPayments(totalInTaxYear = None,
                            toNonUkCharities = Some(100)))),
        Map("/giftAidPayments" -> UNDEFINED_REQUIRED_ELEMENT)
      )
    }
    ...
    "reject if carriedBackToPreviousTaxYear is provided, but totalInTaxYear is not present" in {
      assertValidationError[CharitableGiving](
        CharitableGiving(
          giftAidPayments = Some(
            GiftAidPayments(totalInTaxYear = None,
                            carriedBackToPreviousTaxYear = Some(100)))),
        Map("/giftAidPayments" -> UNDEFINED_REQUIRED_ELEMENT)
      )
    }
    ...
    "reject if oneOff is bigger than totalInTaxYear" in {
      assertValidationError[CharitableGiving](
        CharitableGiving(
          giftAidPayments = Some(
            GiftAidPayments(totalInTaxYear = Some(100), oneOff = Some(101)))),
    ...
    "reject if toNonUkCharities is bigger than totalInTaxYear" in {
      assertValidationError[CharitableGiving](
        CharitableGiving(
          giftAidPayments = Some(
            GiftAidPayments(totalInTaxYear = Some(100),
                            toNonUkCharities = Some(101)))),
        Map("/giftAidPayments" -> MAXIMUM_AMOUNT_EXCEEDED)
      )
    }
    ...
    "reject if carriedBackToPreviousTaxYear is bigger than totalInTaxYear" in {
      assertValidationError[CharitableGiving](
        CharitableGiving(
          giftAidPayments = Some(
            GiftAidPayments(totalInTaxYear = Some(100),
                            carriedBackToPreviousTaxYear = Some(101)))),
        Map("/giftAidPayments" -> MAXIMUM_AMOUNT_EXCEEDED)
      )
    }
