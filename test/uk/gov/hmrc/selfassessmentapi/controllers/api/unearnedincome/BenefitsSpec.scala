...
class BenefitsSpec extends JsonSpec {
  ...
  "format" should {
    ...
    "round trip valid Benefit json" in {
      roundTripJson(
        Benefit(`type` = JobSeekersAllowance,
                amount = BigDecimal(2000.49),
                taxDeduction = BigDecimal(300.00)))
    }
  }
  ...
  "validate" should {
    "reject invalid Benefit type" in {
      ...
      assertValidationError[Benefit](json,
    ...
    "reject amounts with more than 2 decimal values" in {
      Seq(BigDecimal(1000.123),
        ...
        assertValidationError[Benefit](
          Benefit(`type` = JobSeekersAllowance,
                  amount = testAmount,
                  taxDeduction = BigDecimal(304.00)),
          Map("/amount" -> INVALID_MONETARY_AMOUNT),
          "Expected invalid benefits amount with more than 2 decimal places")
      }
    }

    "reject negative amount" in {
      val benefits = Benefit(`type` = JobSeekersAllowance,
                             amount = BigDecimal(-1000.12),
                             taxDeduction = BigDecimal(300.09))
      assertValidationError[Benefit](benefits,
                                     Map("/amount" -> INVALID_MONETARY_AMOUNT),
    ...
    "reject tax deductions greater than the amount" in {
      val benefits = Benefit(`type` = JobSeekersAllowance,
                             ...
                             taxDeduction = BigDecimal(6000.00))
      assertValidationError[Benefit](
        benefits,
        Map("" -> INVALID_TAX_DEDUCTION_AMOUNT),
        "Expected tax deductions amount greater than amount")
    }
  }
}
