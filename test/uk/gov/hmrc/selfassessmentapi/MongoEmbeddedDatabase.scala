...
trait MongoEmbeddedDatabase
  ...
  private val mongoUri = sys.env.getOrElse(
    "MONGO_TEST_URI",
    s"mongodb://$localhost:$embeddedPort/self-assessment-api")
  ...
  lazy protected val mongoClient =
    MongoClient("localhost", if (useEmbeddedMongo) embeddedPort else diskPort)(
      "self-assessment-api")
  ...
  protected def stopEmbeddedMongo() = {
    Try {
      ...
      case ex: Throwable =>
        Logger.info(s"MONGO_STOP_FAILED: Couldn't kill mongod process! : $ex")
    }
  }
  ...
  protected def clearMongoCollections() = {
    List("selfEmployments",
         "employments",
         "selfAssessments",
         "jobHistory",
         "liabilities").foreach { coll =>
