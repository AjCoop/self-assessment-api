...
class SourceSummaryHalLinksSpec extends BaseFunctionalSpec {
  ...
  private val conf: Map[String,
                        Map[SourceId, Map[SourceId, Map[SourceId, Any]]]] =
    Map(
      "Test" ->
        Map("feature-switch" ->
          Map(
  ...
  "Request to create self-employments" should {
    "have Hal links for incomes" in {
      given()
        .userIsAuthorisedForTheResource(saUtr)
        .when()
        .post(s"/$saUtr/$taxYear/self-employments",
              Some(SelfEmployments.example()))
        .thenAssertThat()
        .statusIs(201)
        .contentTypeIsHalJson()
        .bodyHasSummaryLink(SelfEmployments,
                            selfemployment.SummaryTypes.Incomes,
                            saUtr,
                            taxYear)
    }
    ...
    "not have Hal links for expenses" in {
      given()
        ...
        .statusIs(201)
        .contentTypeIsHalJson()
        .bodyDoesNotHaveSummaryLink(SelfEmployments,
                                    selfemployment.SummaryTypes.Expenses,
                                    saUtr,
                                    taxYear)
    }
  }
  ...
  "Request to update self-employments" should {
    "have Hal links for incomes" in {
      given()
        ...
        .statusIs(201)
        .when()
        .put(s"/$saUtr/$taxYear/self-employments/%sourceId%",
             Some(SelfEmployments.example()))
        .thenAssertThat()
        .statusIs(200)
        .contentTypeIsHalJson()
